...
class AdaptiveFeatureFusion(nn.Module):
    def __init__(self, node_feature_dim, cut_feature_dim):
        super(AdaptiveFeatureFusion, self).__init__()

        self.node_feature_dim = node_feature_dim
        self.cut_feature_dim = cut_feature_dim

        self.node_weight = nn.Parameter(torch.randn(node_feature_dim))
        self.cut_weight = nn.Parameter(torch.randn(cut_feature_dim))

    def forward(self, node_features, cut_features):
        node_features = node_features * self.node_weight
        cut_features = cut_features * self.cut_weight

        fused_features = torch.cat((node_features, cut_features), dim=-1)
