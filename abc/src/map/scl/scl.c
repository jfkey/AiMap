...
int Scl_CommandStime( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilGetoptReset();
    while ( ( c = Extra_UtilGetopt( argc, argv, "XCcapdh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'X':
            ...
            case 'C':
                ...
                criNodeFileStr = argv[globalUtilOptind];
                globalUtilOptind++;
                break;
            case 'c':
                fUseWireLoads ^= 1;
    ...
    Abc_SclTimePerform( (SC_Lib *)pAbc->pLibScl, Abc_FrameReadNtk(pAbc), nTreeCRatio, fUseWireLoads, fShowAll, fPrintPath, fDumpStats, criNodeFileStr);
    return 0;
...
usage:
    ...
    fprintf( pAbc->Err, "\t-C     : file path to dump critical node %s\n",  criNodeFileStr);
