...
static inline void Abc_SclTimeNodePrint( SC_Man * p, Abc_Obj_t * pObj, int fRise, int Length, float maxDelay )
{
    ...
    if( !Abc_ObjIsNode(pObj) ) {
        printf( "%8d : ",           Abc_ObjMapNtkId(pObj) );
        printf( "%8d : ",           Abc_ObjMapNtkPhase(pObj) );
    ...
    printf( "%4d ",             Abc_ObjFanoutNum(pObj) );
    printf( "%-*s ",            Length, pCell ? pCell->pName : "pi" );
    printf( "delay=%6.1f ",     Abc_SclObjTimeMax(p, pObj)  - Abc_SclGetMaxDelayNodeFanins(p, pObj) ); // Abc_MaxFloat (Abc_SclObjTime(p,pObj)->rise, Abc_SclObjTime(p,pObj)->fall)
...
static inline void Abc_SclTimeNodeDump( SC_Man * p, Abc_Obj_t * pObj,    float maxDelay, FILE* labelFile )
{
    SC_Cell * pCell = Abc_ObjIsNode(pObj) ? Abc_SclObjCell(pObj) : NULL;
    // printf( "%8d : ",           Abc_ObjId(pObj) );
    ...
    fprintf( labelFile, "%.3f",  maxDelay );
    fprintf( labelFile, "\n" );
}


void Abc_SclTimeNtkPrint( SC_Man * p, int fShowAll, int fPrintPath, char* cirNodeFileStr )
{
    int fReversePath = 1;
    ...
    Abc_NtkSetReportDelay(p->pNtk, maxDelay);
...
//        printf( "Timing information for all nodes: \n" );
    ...
    // there is a bug when computing the critical path on s298 circuit.
    if ( fPrintPath )
    {
...
//        printf( "Critical path: \n" );
    ...
    // dump the critical path into the label file
    if (!strcmp(cirNodeFileStr, ""))  return;
    FILE * labelFile = fopen( cirNodeFileStr, "w" );
    if ( labelFile == NULL )
...
//        Vec_Ptr_t * vPath;
////        Abc_NtkForEachCo( p->pNtk, pObj, j ) {
////            pPivot = Abc_NtkPo(p->pNtk, j);
//
//        // print timing
//        pObj = Abc_ObjFanin0(pPivot);
//        while ( pObj && Abc_ObjIsNode(pObj) )//&& Abc_ObjIsNode(pObj) )
//        {
//            Abc_SclTimeNodeDump( p, pObj, maxDelay, labelFile);
//            pPrev = pObj;
//            pObj = Abc_SclFindMostCriticalFanin( p, &fRise, pObj );
//        }
//
//        Vec_PtrFree( vPath );
        ...
        fclose( labelFile );
        printf( "Dumped labels into text file \"%s\".\n", cirNodeFileStr);
    }
...
***********************************************************************/
void Abc_SclTimePerformInt( SC_Lib * pLib, Abc_Ntk_t * pNtk, int nTreeCRatio, int fUseWireLoads, int fShowAll, int fPrintPath, int fDumpStats, char* cirNodeFileStr )
{
    SC_Man * p;
    p = Abc_SclManStart( pLib, pNtk, fUseWireLoads, 1, 0, nTreeCRatio );
    Abc_SclTimeNtkPrint( p, fShowAll, fPrintPath, cirNodeFileStr);
    if ( fDumpStats )
...
***********************************************************************/
void Abc_SclTimePerform( SC_Lib * pLib, Abc_Ntk_t * pNtk, int nTreeCRatio, int fUseWireLoads, int fShowAll, int fPrintPath, int fDumpStats, char*criNodeFileStr )
{
    Abc_Ntk_t * pNtkNew = pNtk;
    if ( pNtk->nBarBufs2 > 0 )
        pNtkNew = Abc_NtkDupDfsNoBarBufs( pNtk );
    Abc_SclTimePerformInt( pLib, pNtkNew, nTreeCRatio, fUseWireLoads, fShowAll, fPrintPath, fDumpStats, criNodeFileStr );
    if ( pNtk->nBarBufs2 > 0 )
