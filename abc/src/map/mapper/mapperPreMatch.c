/**CFile****************************************************************
  ...
  FileName    [mapperPreMatch.c]

  PackageName [MVSIS 1.3: Multi-valued logic synthesis system.]

  Synopsis    [Generic technology mapping engine; Using learning model to predict the delay of each supergate]
  ...
  Revision    [$Id: mapperPreMatch.c,v 1.7 2023/5/10 14:58:10 satrajit Exp $]
...
***********************************************************************/
void Map_MatchCleanPre( Map_Match_t * pMatch )
{
    ...
    pMatch->tArrive.EstWorst  = MAP_FLOAT_LARGE; // unassigned
...
***********************************************************************/
int Map_MatchComparePre( Map_Man_t * pMan, Map_Match_t * pM1, Map_Match_t * pM2, int fDoingArea )
{
...
//        return 0;
    if ( !fDoingArea )
    {
        ...
        if ( pM1->tArrive.EstWorst  < pM2->tArrive.EstWorst - pMan->fEpsilon )
            return 0;
        if ( pM1->tArrive.EstWorst > pM2->tArrive.EstWorst + pMan->fEpsilon )
            return 1;
...
int Map_MatchNodeCutPre( Map_Man_t * p, Map_Node_t * pNode, Map_Cut_t * pCut, int fPhase, float fWorstLimit )
{
    ...
    for ( pSuper = pMatch->pSupers, Counter = 0; pSuper; pSuper = pSuper->pNext, Counter++ )
    {
        ...
        for ( i = 0; i < (int)pSuper->nPhases; i++ )
        {
            ...
            if ( Map_MatchComparePre( p, &MatchBest, pMatch, p->fMappingMode ) )
            {
                MatchBest = *pMatch;
                // if we are mapping for delay, the worst-case limit should be reduced
                if ( p->fMappingMode == 0 )
                    fWorstLimit = MatchBest.tArrive.EstWorst;
            }
...
***********************************************************************/
void Map_MappingSetPiArrivalTimesPre( Map_Man_t * p )
{
    Map_Node_t * pNode;
    int i;
    for ( i = 0; i < p->nInputs; i++ )
    {
        ...
        if ( Scl_ConIsRunning() )
        {
            ...
            pNode->tArrival[1].EstWorst = Time;
        ...
        pNode->tArrival[1].EstWorst += p->pNodeDelays ? p->pNodeDelays[pNode->Num] : 0;
        ...
        pNode->tArrival[0].EstWorst = MAP_MAX(pNode->tArrival[0].Rise, pNode->tArrival[0].Fall);
    }
}
...
***********************************************************************/
float Map_TimeMatchWithInverterPre( Map_Man_t * p, Map_Match_t * pMatch )
{
//    Map_Time_t tArrInv;
...
//    return tArrInv.Worst;
    return MAP_MAX(pMatch->tArrive.EstWorst + p->pSuperLib->tDelayInv.Rise, pMatch->tArrive.EstWorst + p->pSuperLib->tDelayInv.Fall);
}
...
***********************************************************************/
void Map_NodeTryDroppingOnePhasePre( Map_Man_t * p, Map_Node_t * pNode )
{
    ...
    tWorst0Using1 = Map_TimeMatchWithInverterPre( p, pMatchBest1 );
    tWorst1Using0 = Map_TimeMatchWithInverterPre( p, pMatchBest0 );
    ...
    if ( p->fMappingMode == 0 && p->DelayTarget < ABC_INFINITY )
    {
        ...
        if ( pMatchBest0->tArrive.EstWorst > tWorst0Using1 + p->fEpsilon )
            pNode->pCutBest[0] = NULL;
        else if ( pMatchBest1->tArrive.EstWorst > tWorst1Using0 + p->fEpsilon )
    ...
    if ( p->fMappingMode == 2 )
    {
        fUsePhase0 = (pNode->tRequired[1].EstWorst > tWorst1Using0 + 3*p->pSuperLib->tDelayInv.Worst + p->fEpsilon);
        fUsePhase1 = (pNode->tRequired[0].EstWorst > tWorst0Using1 + 3*p->pSuperLib->tDelayInv.Worst + p->fEpsilon);
    }
    else if ( p->fMappingMode == 3 || p->fMappingMode == 4 )
    {
        fUsePhase0 = (pNode->tRequired[1].EstWorst > tWorst1Using0 + p->fEpsilon);
        fUsePhase1 = (pNode->tRequired[0].EstWorst > tWorst0Using1 + p->fEpsilon);
    }
...
***********************************************************************/
void Map_NodeTransferArrivalTimesPre( Map_Man_t * p, Map_Node_t * pNode )
{
    ...
    else if ( pNode->pCutBest[0] )
    {
        ...
        pNode->tArrival[1].EstWorst =  pNode->tArrival[0].EstWorst + MAP_MAX(p->pSuperLib->tDelayInv.Rise, p->pSuperLib->tDelayInv.Fall);
    }
    else if ( pNode->pCutBest[1] )
    {
        ...
        pNode->tArrival[0].Fall  = pNode->tArrival[1].Rise + p->pSuperLib->tDelayInv.Fall;
        pNode->tArrival[0].Worst = MAP_MAX(pNode->tArrival[0].Rise, pNode->tArrival[0].Fall);
        pNode->tArrival[0].EstWorst =  pNode->tArrival[1].EstWorst + MAP_MAX(p->pSuperLib->tDelayInv.Rise, p->pSuperLib->tDelayInv.Fall);
    }
    else
    {
        assert( 0 );
...
***********************************************************************/
int Map_MappingMatchesPre( Map_Man_t * p )
{
    ...
    if ( p->fMappingMode == 0 )
        Map_MappingSetPiArrivalTimesPre( p );
...
//            printf("maped: node(%d), index(%d) AND\n", pNode->Num, i);
//        }
//        if (Map_NodeIsBuf(pNode)){
//            printf("maped: node(%d), index(%d) Buf\n", pNode->Num, i);
//        }
//        if (Map_NodeIsVar(pNode)){
//            printf("maped: node(%d), index(%d) Var\n", pNode->Num, i);
//        }
//        if (Map_NodeIsConst(pNode)){
//            printf("maped: node(%d), index(%d) Const\n", pNode->Num, i);
//        }
        ...
        if ( !Map_MatchNodePhasePre( p, pNode, 0 ) )
        {
            Extra_ProgressBarStop( pProgress );
            return 0;
        }
        // match positive phase
        if ( !Map_MatchNodePhasePre( p, pNode, 1 ) )
        {
            Extra_ProgressBarStop( pProgress );
            return 0;
        }
        ...
        Map_NodeTryDroppingOnePhasePre( p, pNode );
        // set the arrival times of the node using the best cuts
        Map_NodeTransferArrivalTimesPre( p, pNode );
